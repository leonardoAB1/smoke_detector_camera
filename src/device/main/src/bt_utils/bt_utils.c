...
static uint32_t connection_handle;
...
void bt_printf(const char *format, ...)
{
    ...
    if (connection_handle == NULL) {
        printf("Bluetooth connection handle is not valid. Cannot send data.\n");
        return;
    }
    ...
    // Print the formatted string to the console for debugging (optional)
    //printf("Formatted string: %s", buffer);

    // Send the formatted string through Bluetooth
    esp_spp_write(connection_handle, strlen(buffer), (uint8_t *)buffer);
}
...
static void print_speed(void)
{
    ...
    ESP_LOGI(BT_TAG, "speed(%fs ~ %fs): %f kbit/s" , time_old_s, time_new_s, speed);
    data_num = 0;
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_INIT_EVT:
        if (param->init.status == ESP_SPP_SUCCESS) {
            ...
            ESP_LOGE(BT_TAG, "ESP_SPP_INIT_EVT status:%d", param->init.status);
        }
        break;
        /*********************************************************************************************************/
    ...
    case ESP_SPP_CLOSE_EVT:
        ESP_LOGI(BT_TAG, "ESP_SPP_CLOSE_EVT status:%d handle:%"PRIu32" close_by_remote:%d", param->close.status,
                 param->close.handle, param->close.async);
        break;
        /*********************************************************************************************************/
    case ESP_SPP_START_EVT:
        if (param->start.status == ESP_SPP_SUCCESS) {
            ESP_LOGI(BT_TAG, "ESP_SPP_START_EVT handle:%"PRIu32" sec_id:%d scn:%d", param->start.handle, param->start.sec_id,
                     param->start.scn);
            esp_bt_dev_set_device_name(DEVICE_NAME);
            esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
        } else {
            ESP_LOGE(BT_TAG, "ESP_SPP_START_EVT status:%d", param->start.status);
        }
        break;
        /*********************************************************************************************************/
    ...
    case ESP_SPP_DATA_IND_EVT:
         ...
         */
        ESP_LOGI(BT_TAG, "ESP_SPP_DATA_IND_EVT len:%d handle:%lu",
                 param->data_ind.len, param->data_ind.handle);
        if (param->data_ind.len < 128) {
            esp_log_buffer_hex("", param->data_ind.data, param->data_ind.len);
            //echo message
            //esp_spp_write(param->data_ind.handle, param->data_ind.len, param->data_ind.data);
            //bt_printf("This is a test message: %d, %s", 42, "Hello!");
            //bt_printf("This is another test message");
        }
        //if needed, here goes actions triggered by message received
#else
...
#endif /*SPP_SHOW_MODE == SPP_SHOW_DATA*/
        break;
        /*********************************************************************************************************/
    ...
    case ESP_SPP_SRV_OPEN_EVT:
        //This event indicates that the SPP server has been opened, 
        //and the connection handle is provided as a parameter.
        // Store the connection handle for future reference
        connection_handle = param->srv_open.handle;
        ESP_LOGI(BT_TAG, "ESP_SPP_SRV_OPEN_EVT status:%d handle:%"PRIu32", rem_bda:[%s]", param->srv_open.status,
...
void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_AUTH_CMPL_EVT:{
        if (param->auth_cmpl.stat == ESP_BT_STATUS_SUCCESS) {
            ESP_LOGI(BT_TAG, "authentication success: %s bda:[%s]", param->auth_cmpl.device_name,
    ...
    case ESP_BT_GAP_MODE_CHG_EVT:
        ESP_LOGI(BT_TAG, "ESP_BT_GAP_MODE_CHG_EVT mode:%d bda:[%s]", param->mode_chg.mode,
...
void init_bluetooth(esp_err_t ret){
    ...
    ESP_LOGI(BT_TAG, "Own address:[%s]", bda2str((uint8_t *)esp_bt_dev_get_address(), bda_str, sizeof(bda_str)));
}
